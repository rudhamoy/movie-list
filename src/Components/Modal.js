  ...
  return (
    <div onClick={closeModal} className='bg-[#00000090] flex justify-center items-center fixed top-0 left-0 right-0 bottom-0 z-40'>
      {movieInfoLoading ? (
        ...
        <div onClick={e => e.stopPropagation()} className='bg-gray-100 p-2 rounded-md w-[17rem] sm:w-[40rem] z-50 flex flex-col sm:flex-row gap-x-5 relative'>
          <div className='h-[14rem] sm:h-[24rem] w-[16rem] overflow-hidden rounded-md'>
            <img src={movieInfo.Poster} alt={movieInfo.Title} className='rounded-md ' />
          </div>
          <div className='text-sm text-gray-700 sw-[15rem] sm:w-[17rem] space-y-2'>
            ...
            <p className='hidden sm:block'><span className='font-semibold'>Plot:</span> {movieInfo.Plot}</p>
          </div>
